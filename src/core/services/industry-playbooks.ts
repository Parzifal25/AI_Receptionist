 ...
 * free-text industry/description, so tenants get expert behaviour with zero
 * configuration. Pure data + one pure matcher — fully unit-testable.
 */
...
export interface IndustryPlaybook {
  ...
  notes: string[];
  /** A hard line the receptionist must never cross (advice, promises, etc.). */
  compliance?: string;
}
 ...
 * description. Returns null when no playbook matches — the generic
 * receptionist behaviour still applies.
 */
export function matchIndustryPlaybook(
  ...
  return INDUSTRY_PLAYBOOKS.find((p) => p.match.test(haystack)) ?? null;
}
